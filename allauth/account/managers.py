...
if getattr(settings, "VARY_TOP_LEVEL_MODEL_BY_SITE", False):
    ...
    class EmailAddressManager(EmailAddressManagerMixin, CurrentSiteManager):
        ...
        def get_queryset(self):
            if not (
                hasattr(self, "_get_field_name")
                and getattr(settings, "STAFF_ACCOUNT_MULTI_SITE", False)
            ):
            ...
            return (
                ...
                .get_queryset()
                .filter(
                    models.Q(**{self._get_field_name() + "__id": settings.SITE_ID})
    ...
    class EmailAddressManager(EmailAddressManagerMixin, models.Manager["EmailAddress"]):
        pass
